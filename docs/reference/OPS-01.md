...
| 无崩溃监控（依赖商店报告） | WSSE、场景触发精度、AI 成本均不可见 | 运营瞎 |
...
- **OMTM**：场景识别精准率 > 90%（用户触发后的场景归类与表达卡准确度）
...
| # | 能力 | 交付物 | 验收标准 | 状态 |
|---|---|---|---|---|
| 1 | **AI 代理层** | 客户端 AI 网关已封装（URL 可配，Supabase 就绪后切换）；Edge Function 待部署 | 客户端无任何 key；按用户配额 + 速率限制 + 熔断生效 | 🔶 客户端完成，服务端待 Supabase |
| 2 | **合规三件套** | 隐私政策、用户协议、数据清单（App 内页 + `OPS-02.md`） | 审核可提交；页面可访问 | ✅ 2026-08-03 |
| 3 | **上架链路** | eas build/submit 自动化、TestFlight 内测分发 | 一键出 TestFlight 包 | ⬜ |
| 4 | **监控补全** | 崩溃与性能监控方案（Sentry 已移除，改用商店报告 + 自建埋点） | 识别/渲染耗时可见 | ⬜ |
...
| 6 | **场景包体系（枢纽）** | `src/packs/`：版本化场景包（10 场景 / 26 国）+ 本地缓存 + 远程下发接口（URL 待配置） | 新增城市/修正词条**不发版**即可生效；旧包可回滚 | ✅ 2026-08-03（本地+接口，远程待运营后台） |
| 7 | **订阅商业化** | iOS IAP（Day Pass / Weekly Pro）+ 服务端 receipt 验证 + 权益门控 | 订阅可购、可恢复、可退款感知；绕过验证无法解锁 | ⬜ |
| 8 | **行为分析** | 事件埋点：WSSE、场景识别精度、卡片使用率、AI 单次成本 | 北极星指标可每日导出 | ⬜ |

### P2 增长运营（2-3 月）

| # | 能力 | 说明 | 状态 |
|---|---|---|---|
| 9 | 分享/邀请 | 表达卡截图分享已上线；邀请码/链接待做 | 🔶 卡片分享 ✅ |
| 10 | 落地页 + ASO | 官网（兼隐私政策宿主）、关键词、商店截图 | ⬜ |
| 11 | 免费/付费分层 | 每日免费 N 次 AI 识别 → 订阅解锁无限 | ⬜ |
| 12 | 城市内容 SOP | 曼谷 → 东京 → 首尔/新加坡；每城一套（打车/餐饮/退税/地铁/SOS + 本地规则），周更节奏 | 🔶 已泛化 26 国数据，城市级 SOP 待建 |
...
> 依赖图与明细见 [`DEPENDENCIES-01.md`](DEPENDENCIES-01.md)，本文件不重复维护。
> 关键前置：P0 AI 代理 → P1 订阅/分析；P1 场景包 → P2 内容运营；P1 匿名账号 → P1 订阅/分析。
